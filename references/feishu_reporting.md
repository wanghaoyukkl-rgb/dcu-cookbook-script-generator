...
表格写入采用 upsert 语义，在对应框架工作表内以固定脚本文件名为唯一键：

- 表格中不存在该文件名时新增记录。
- 表格中已存在该文件名时，以最新绝对路径、模型路径、卡型、时间戳和 KVCache-FP8 信息覆盖旧记录。
- 如果历史追加操作或路径解析变化留下了多条同文件名记录，只保留一条最新记录并清理其余重复项。
- 自定义输出目录发生变化，以及 `/public/home` 与 `/public2/home` 等路径别名变化，不会产生重复记录。
- 不得仅按模型名删除记录；固定文件名包含框架、模型、卡型和卡数，同一模型的其它有效部署组合应分别保留。
...
4. 只有命令返回 `status: reported`，并同时给出 table 与 message 结果，才汇报飞书闭环成功。`table.action` 为 `created` 表示首次新增，为 `updated` 表示已替换同文件名旧记录。
